...
- 对象类型:对象类型的变量都位于堆上
  - 堆上的对象,必须需要手动释放,或者调用autorelease,交给autoreleasePool释放,才能释放
    - 下面例子中 obj 指针本身局部变量,在栈上,出作用域自动释放;
    - alloc init 出来的对象(obj指向的内存),位于堆上,如果不经过手动释放或调用 autprelease,不会释放
  ...
  ```  
- 平时遇到的函数,有的返回的对象是接收者自动持有的,有的接受者不自动持有,怎么实现的呢?
  - 自己生成持有
  ```
    - (id)testCount {
      id obj = [[NSArray alloc] init];
  ...
  ```
    - (id)testCount {
        id obj = [[NSArray alloc] init];
        [obj autorelease];//被放进了线程(可能是主线程)的runloop对应的releasepool
        return obj;
    }
    id temp = [self testCount];//必须retain 才能保证持有对象      
  ``` 
...
- OC中的对象是否释放,采用引用计数机制:
  ...
  - retain——引用计数+1
  - release——引用计数-1
  - Autorelease 和 autoreleasePool 后面详细介绍
- 引用计数管理内存需要注意的问题:
  - 释放不是自己拥有的对象,会造成crash
    - 自己拥有的对象,释放(release)两次,第二次会crash,因为此时已经不再拥有对象
      - 解决办法:release释放后,对指针置为nil
    - 通过函数获取的不是自己持有的对象,进行释放会Crash;上面第二个例子
